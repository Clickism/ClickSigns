...
public class FieldWidget extends NestedWidget {
    public FieldWidget(int x, int y, Component header, Component text) {
        super(x, y);
        ...
        var textWidget = new StringWidget(x + padding, y + lineOffset + padding + 10, 0, 0, text, GuiUtils.font());
        textWidget.alignLeft();
        addChild(textWidget);

        var width = Math.max(GuiUtils.font().width(header), GuiUtils.font().width(text)) + padding * 2;
        var height = (textWidget.getY() + textWidget.getHeight() + lineOffset) - y + padding;
